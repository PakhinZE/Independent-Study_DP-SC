...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iter", type=int, default=500, help="the number of iterations to run"
    )
    ...
    for hpath in args.hypothesis:
        ...
        for j in range(num_iterations):
            random.seed(j * 101)
            indices.append(
                [random.randint(0, len(args.reference) - 1) for i in range(len(hyp))]
            )
        ...
        iter_stats = [[0 for i in range(2 * args.n + 2)] for j in range(num_iterations)]
        ...
        for i, h in enumerate(hyp):
            ...
            for j in range(num_iterations):
                ...
                if this_stats is None:
                    ...
                    stats_by_ref[ref] = this_stats  # type: ignore
        ...
        if args.debug:
            ...
            print(
                get_gleu_stats([gleu_calculator.gleu(stats) for stats in iter_stats])[0]
            )
