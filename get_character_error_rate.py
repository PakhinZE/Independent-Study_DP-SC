...
# %%
labels = class_model.readlines(label_path)

# %%
preds = class_model.readlines(pred_path)
score = char_error_rate(preds, labels)
print(f"Score For Spell Correction: {score}")
del preds

# %%
dp_preds = class_model.readlines(dp_pred_path)
...
# %%
mask_labels = class_model.readlines(mask_label_path)

# %%
mask_preds = class_model.readlines(mask_pred_path)
mask_score = char_error_rate(mask_preds, mask_labels)
print(f"Score For Memoization: {mask_score}")
del mask_preds
# %%
dp_mask_preds = class_model.readlines(dp_mask_pred_path)
